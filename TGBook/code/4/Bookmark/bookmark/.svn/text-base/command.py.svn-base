...
# symbols which are imported by "from bookmark.command import *"
...
from bookmark.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("bookmark"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="bookmark.config")
...
def bootstrap():
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="Bookmark %s" % version)
    ...
    _read_config(args)
    from bookmark.model import bootstrap_model
