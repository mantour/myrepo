...
__connection__ = hub = PackageHub('sqlplayground')


# your data model

class User(SQLObject):
    user_name = StringCol(alternateID=True, length = 20)
    first_name = StringCol()
    middle_initial = StringCol(length=1, default=None)
    last_name = StringCol()
...
def create_tables(drop_all=False):
    ...
    model = get_model()
    if not model:
        from sqlplayground.command import ConfigurationError
