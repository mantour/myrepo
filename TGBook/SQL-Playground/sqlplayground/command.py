...
# symbols which are imported by "from sqlplayground.command import *"
...
from sqlplayground.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("sqlplayground"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="sqlplayground.config")
...
def bootstrap():
    ...
    By default this runs the function 'sqlplayground.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-sqlplayground = sqlplayground.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="SQL-Playground %s" % version)
    ...
    _read_config(args)
    from sqlplayground.model import bootstrap_model
