...
# symbols which are imported by "from hello.command import *"
...
from hello.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("hello"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="hello.config")
...
def bootstrap():
    ...
    By default this runs the function 'hello.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-hello = hello.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="hello %s" % version)
    ...
    _read_config(args)
    from hello.model import bootstrap_model
...
def start():
    ...
    from hello.controllers import Root
